...
<body>
    ...
    <table id="myTable">
        ...
        <tbody>
            <?php
                $sql = "SELECT `complaint_info`.`U_id`,`complaint_info`.`C_id`,`complaint_info`.`C_state`,`complaint_info`.`C_type`,`complaint_info`.`C_doc`,`complaint_info`.`C_desc`,`case_details`.`PS_id`,`case_details`.`P_id`,`complaint_info`.`Status` FROM `complaint_info` LEFT JOIN `case_details` on `complaint_info`.`C_id` = `case_details`.`C_id`;";
                ...
                if($count>0)
                {
                    ...
                    {
                        $ps=$row['PS_id'];
                        if($row['PS_id']==NULL && $row['Status']!='Rejected')
                        {
                            $ps="Not Assigned";
                        }
                        else if($row['PS_id']==NULL && $row['Status']=='Rejected')
                        {
                            $ps="-----";
                        }
                        $p=$row['P_id'];
                        if($row['P_id']==NULL && $row['Status']!='Rejected')
                        {
                            $p="Not Assigned";
                        }
                        else if($row['P_id']==NULL && $row['Status']=='Rejected')
                        {
                            $p="----";
                        }
                        echo "<tr>
    ...
    <section class="my_form">
		<div class="center">
        <h1>Case Rejection Form</h1>
        <form action="<?php echo $_SERVER["PHP_SELF"]?>" method="post">
        <select name="cid" id="cid" required class="Drops" >
                <option value="">Select Complaint ID</option>
                <?php
                     $sql = "SELECT `complaint_info`.`U_id`,`complaint_info`.`C_id` FROM `complaint_info` INNER JOIN `case_details` on `complaint_info`.`C_id` = `case_details`.`C_id` AND `complaint_info`.`Status`!='Rejected' AND `complaint_info`.`Status`!='Complete';";
                     $result = mysqli_query($conn,$sql);
                     if(!$result)
                    {
                        echo " error";
            ...
            <br>
            <textarea name="rejdesc" id="" cols="5" rows="8" required placeholder="Enter the Reason for Rejection"></textarea><br>
        ...
        if(isset($_REQUEST['submitupdate']))
        {
            ...
            $cid1= $array_id[0];
            $uid1 = $array_id[1];
            $rejdesc=$_POST['rejdesc'];
            // echo $cid1.$uid1;
            // echo $rejdesc;
            $sql= "INSERT INTO `rejected_cases`(`C_id`,`U_id`,`Rejec_desc`) VALUES('$cid1','$uid1','$rejdesc');";
            $result = mysqli_query($conn,$sql);
            if(!$result)
            {
                echo " error";
            }
            $sql1= "UPDATE `complaint_info` SET `Status`='Rejected' WHERE `C_id`='$cid1'";
            $result1 = mysqli_query($conn,$sql1);
            if(!$result1)
            {
                echo " error";
            }
            $sql2= "INSERT INTO `case_update` (`C_id`,`U_id`,`CU_desc`) VALUES ('$cid1','$uid1','Case Rejected');";
            $result2 = mysqli_query($conn,$sql2);
            if(!$result2)
            {
                echo " error";
            }
            $sql3= "UPDATE `case_details` SET `PS_id`=NULL,`P_id`=NULL WHERE `C_id`='$cid1'";
            ...
            if(!$result3)
            {
                ...
                echo "<script> location.href='HComplaint.php'</script>";
            }
        }
    ?>
	</section>
