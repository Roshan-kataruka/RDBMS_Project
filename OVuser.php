...
<body>
    ...
    <table id="myTable">
        ...
        <tbody>
            <?php
                $sql = "SELECT * FROM `user_info` WHERE `U_id` IN (SELECT `case_details`.`U_id` FROM `case_details` INNER JOIN `officier_info` ON `case_details`.`P_id` = `officier_info`.`P_id` AND `officier_info`.`P_id`='$pid');";
